...
class TestMutualInfo(unittest.TestCase):
    ...
    def test_trivial_zero_case(self):
        ...
        basis = Series([0,0,1,1]).astype(bool)
        no_mutual_info = Series([0,1,0,1]).astype(bool)
        self.assertEqual(compare.binary_mutual_information(basis, no_mutual_info), 0)
...
class TestEntropy(unittest.TestCase):
    ...
    def test_basic_case(self):
        ...
        self.assertTrue(np.isclose([compare.entropy(dataSet1)], [0.81127812445913283]))
        self.assertTrue(np.isclose([compare.entropy(dataSet2)], [0.81127812445913283]))
